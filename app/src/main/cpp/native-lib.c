 ...
 */
jstring Java_com_actor_testapplication_utils_JavaCallC_stringFromJNI(JNIEnv* env, jobject instance) {
...
Java_com_actor_testapplication_utils_JavaCallC_add(JNIEnv* env, jobject instance, jint x, jint y){
    LOGD("x + y =%d\n", x+y);
    LOGI("也可以只写一个参数");
    LOGE("x + y =%s\n", "y+z");
    return x+y;
...
Java_com_actor_testapplication_utils_JavaCallC_add10(JNIEnv* env, jobject instance, jintArray arr_) {
    //1. 获取数组的大小
