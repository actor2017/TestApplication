package com.actor.testapplication.novel;
...
//            "&rsv_t=e042GcwGqE0nAa19rwnTUWJDLfOHxHckzpcdn2ZSryVINVpDaS2B61HJsog"
    ...
    public void findNovelWeb(String novelName, @IntRange(from = 1, to = 99) int page, OnFindElementsListener listener) {
        ...
        LogUtils.errorFormat("开始请求, start=%d", start);
        int pn = (page - 1) * 10;
...
//                Document document = Jsoup.parse(htmlStr);
                ...
                LogUtils.errorFormat("请求&解析Document对象完成, end=%d, 耗时=%d", end, end - start);

                //<a标签, 且class="siteLink_9TPP3"
                Elements select = document.select("a[class=siteLink_9TPP3]");

                parseTime = System.currentTimeMillis();
                LogUtils.errorFormat("查找<a标签完成, end=%d, 耗时=%d", parseTime, parseTime - end);

                if (parseTime - end <= 10) {
                    LogUtils.errorFormat("可能出错了, 请求内容: %s", document.toString());
...
//            LogUtils.errorFormat("val=%s, data=%s, html=%s, ownText=%s, wholeText=%s, text=%s", val, data, html, ownText, wholeText, text);
...
//                urls.put(url, false);
        ...
        LogUtils.errorFormat("遍历完成<a标签完成, end=%d, 耗时=%d", forTime, forTime - parseTime);
        ...
        ThreadUtils.executeByIo(new MySimpleTask<Document>(webName, listener) {
            ...
            public void onSuccess(Document result) {
                ...
                if (TextUtils.isEmpty(baseUri)) {
                    LogUtils.errorFormat("解析的baseUri = null!!!, result = %s", result.toString());
                } else {
                    /**
                     ...
                     */
//                LogUtils.errorFormat("baseUri = %s", baseUri);
