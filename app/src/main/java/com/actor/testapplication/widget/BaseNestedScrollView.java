...
public class BaseNestedScrollView extends NestedScrollView {
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        LogUtils.formatError("target=%s, dx=%d, dy=%d, consumed=%s", true, target, dx, dy, Arrays.toString(consumed));
    }

    @Override
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        super.onScrollChanged(l, t, oldl, oldt);
        LogUtils.formatError("l=%d, t=%d, oldl=%d, oldt=%d", true, l, t, oldl, oldt);
        mScrollY = t;
    }
}
