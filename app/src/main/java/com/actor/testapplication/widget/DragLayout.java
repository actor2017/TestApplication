...
public class DragLayout extends FrameLayout {
    ...
    private ViewDragHelper viewDragHelper;//v4包
    private View           fl_sec;
    ...
    private void init() {
        viewDragHelper = ViewDragHelper.create(this, new ViewDragHelper.Callback() {
            ...
            public void onViewReleased(View releasedChild, float xvel, float yvel) {
                ...
                if (releasedChild == fl_main && xvel > 200 || fl_main.getLeft() > mainWidth * 0.6 * 0.5 + 0.5) {
                    open();//打开左边侧边栏
                }
                if (xvel < -200 || fl_main.getLeft() < mainWidth * 0.6 * 0.5 + 0.5) {
                    close();//关闭左边侧边栏
                }
            }
        });
    ...
    //打开左边侧边栏
    private void open() {
        viewDragHelper.smoothSlideViewTo(fl_main, (int) (mainWidth * 0.6 + 0.5),0);
    ...
    //关闭左边侧边栏
    private void close() {
        viewDragHelper.smoothSlideViewTo(fl_main,0,0);
        viewDragHelper.continueSettling(true);
        //invalidate();
    }
