...
/**
 * Description: 自定义CardView圆角, 可单独设置某个角是圆角, copy from {@link CardView}, 还未完成...
...
public class MyCardView extends FrameLayout {
    ...
    static {
        if (Build.VERSION.SDK_INT >= 21) {
            IMPL = new MyCardViewApi21Impl();//edited CardViewApi21Impl
    ...
    final Rect mContentPadding = new Rect();//设置内容内边距

    final Rect mShadowBounds = new Rect();//阴影的大小
...
//    }
    ...
    //edited 增加方法
    public void setRadius(int left, int top, int right, int bottom) {
        if (Build.VERSION.SDK_INT >= 21) {
            ((MyCardViewApi21Impl)IMPL).setRadius(mCardViewDelegate, new Rect(left, top, right, bottom));
        }
//        else setRadius();
    }
